...
async function processArenaQueue(): Promise<boolean> {
  try {
    ...
    // soloEntries: single real players eligible for pairing in phase 3.
    ...
    // Phase 2 (30-90 s): require 4 full real-player teams
    ...
    // Phase 3 (>= 90 s): proceed with whatever is available
    ...
    const arenaId = await assembleArenaMatch(syntheticLobby);
    ...
    if (arenaId) {
      ...
      logger.info(
        `Arena match assembled: arenaId=${arenaId}, arenaTeams=${arenaTeams.length}, realPlayers=${totalReal}`,
      );
    }
